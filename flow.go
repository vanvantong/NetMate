...
func (f *Flow) Init(srcip string,
	...
	f.f[FPSP] = new(ValueFeature)
	f.f[BPSP] = new(ValueFeature)
	f.f[FPMP] = new(ValueFeature)
	f.f[BPMP] = new(ValueFeature)
	f.f[FPLP] = new(ValueFeature)
	f.f[BPLP] = new(ValueFeature)
...
func (f *Flow) Add(pkt packet, srcip string) int {
	...
	if f.pdir == P_FORWARD {
		...
		//Calculate fpsp, fpmp, fplp
		if length < 150 {
			f.f[FPSP].Add(1)
		}
		if (length >= 150 && length <= 1000) {
			f.f[FPMP].Add(1)
		}
		if length > 1000 {
			f.f[FPLP].Add(1)
...
func (f *Flow) Export() {
	...
	//fmt.Printf("Number of features:%d", NUM_FEATURES)
	//fmt.Println()
	...
	//fmt.Println()
	//fmt.Printf("FPSP: %f, %d", float32(f.f[FPLP].Get())/float32(f.f[TOTAL_FPACKETS].Get() + f.f[TOTAL_BPACKETS].Get()), f.f[FPLP].Get())
	//fmt.Println()
	//fmt.Printf("BPSP: %f, %d", float32(f.f[BPLP].Get())/float32(f.f[TOTAL_FPACKETS].Get() + f.f[TOTAL_BPACKETS].Get()), f.f[BPLP].Get())
	//fmt.Printf(",%d", f.dscp)
	fmt.Println()
}
